...
さらに，Colab A100 上で GPT-2 による residual stream patching pilot を実行し，真の patch-effect target に対する proxy error と検出性能を測定した．
現時点の合成データでは，contrastive profile を含めても one-pass hidden features のみで完全分類できており，PatchDistill の優位性ではなく，より厳密なデータ設計と held-out framing 評価の必要性を示す結果となった．
\end{abstract}
...
表~\ref{tab:gpt2-pilots} に，真の patch-effect target に対する proxy error と検出性能を示す．
Stress profile では proxy model の patch-effect prediction error は MAE $=0.0277$，MSE $=0.00407$ であった．
HF feature のみの Logistic Regression と，distilled patch feature を加えた Logistic Regression は，どちらも F1 $=1.0$，AUROC $=1.0$ となった．
この結果は優位性の証拠ではなく，むしろ現行 synthetic data が GPT-2 の one-pass 統計だけで分離できるほど容易であることを示す．
...
Proxy error は MAE $=0.0528$，MSE $=0.01265$ であり，HF feature のみの検出器と distilled patch feature を加えた検出器はいずれも F1 $=1.0$，AUROC $=1.0$ であった．
...
これは contrastive framing が patch-effect target の近似を難しくしている可能性を示す一方，features-only detector と distilled detector はどちらも依然として F1 $=1.0$，AUROC $=1.0$ であった．
したがって，contrastive profile はルール特徴の破綻と proxy target の難化を観測するには有用であるが，検出器比較としてはまだ不十分である．

\begin{table}[t]
\centering
\caption{Colab A100 上の GPT-2 residual patching pilot．各 profile で 160 例の one-pass feature を抽出し，24 例に residual stream patching を行った．}
\label{tab:gpt2-pilots}
\begin{tabular}{lrrrrrr}
\toprule
Profile & MAE & MSE & Feat. F1 & Feat. AUROC & Distill F1 & Distill AUROC \\
\midrule
Stress & 0.0277 & 0.00407 & 1.000 & 1.000 & 1.000 & 1.000 \\
Matched & 0.0528 & 0.01265 & 1.000 & 1.000 & 1.000 & 1.000 \\
Contrastive & 0.0902 & 0.03227 & 1.000 & 1.000 & 1.000 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}
...
次の Colab A100 実験では，GPT-2 で確認した pipeline を Qwen2.5-0.5B-Instruct に拡張し，さらに Qwen2.5-1.5B，Gemma-2-2B，7B 級 instruction-tuned model へ段階的に拡張する．
同時に，paraphrased contrastive frames と held-out frame family split を追加し，features-only detector が使う template-order shortcut を抑制する．
