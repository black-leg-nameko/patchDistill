...
\section{初期結果}
表~\ref{tab:surrogate-results} に template split における予備結果を示す．
合成データが単純であるため，ルール特徴と surrogate PatchDistill は完全分類となった．
この結果は手法の優位性を示すものではなく，データ生成，split，評価，proxy，reporting の実験パイプラインが動作していることを確認するための smoke result である．

\begin{table}[t]
\centering
\caption{合成 direct PI データにおける初期 smoke result．}
...
\section{考察}
初期結果から分かることは，現時点の synthetic データでは表層的手がかりが強く，検出器比較としては不十分であるという点である．
...
\begin{enumerate}
  ...
  \item HF モデルで真の patch signature を生成し，proxy error と detector 性能を評価する．
