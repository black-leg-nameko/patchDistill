...
さらに，Colab A100 上で GPT-2 による residual stream patching pilot を実行し，真の patch-effect target に対する proxy error を測定した．
実装監査では HF detector 評価における label leakage を発見・修正し，修正前の detector 成績を無効化した．
その後，paraphrased source-boundary frame を frame family ごとに holdout する contrastive-frame profile を追加し，TF-IDF などの表層 baseline が大きく崩れることを確認した．
修正後 GPT-2 pilot では，distilled patch feature により AUROC が $0.718$ から $0.755$ に向上したが，default threshold では FPR が $1.0$ となり，固定 FPR での再評価が必要である．
\end{abstract}
...
\begin{itemize}
  ...
  \item 初期合成データと GPT-2 pilot により，データ生成，特徴抽出，residual patching，proxy 学習，検出器評価，結果収集までの実験基盤を検証する．
\end{itemize}
...
各 profile について 160 例から one-pass feature を抽出し，positive 24 例に対して residual stream patching を行った．
表~\ref{tab:gpt2-pilots} に，真の patch-effect target に対する proxy error を示す．
実装監査により，初期 pilot の HF detector 学習では numeric feature matrix に label が混入していたことが分かった．
そのため，修正前に得られた features-only detector および distilled detector の F1/AUROC は本稿の証拠として用いない．
以下では residual patching と proxy prediction error のみを，修正後 detector 再実験に向けた preliminary diagnostic として扱う．
...
GPT-2 を用いた matched profile の Colab A100 pilot では，one-pass feature を 160 例から抽出し，24 例に residual patching を行った．
Proxy error は MAE $=0.0528$，MSE $=0.01265$ であった．
このことから，matched profile は手作りルールを崩す一方で，GPT-2 の内部統計や TF-IDF から見える prefix/source-boundary の手がかりはまだ強いと考えられる．
さらに，境界語彙を両クラスで共有し，否定と語順のみを反転させる contrastive profile を追加した．
Local surrogate 実験では，ルール特徴の AUROC は $0.5$ まで低下したが，TF-IDF は依然として F1 $=1.0$，AUROC $=1.0$ であった．
これは，現在の TF-IDF baseline が bigram の語順ショートカットを利用している可能性を示す．
Contrastive profile の GPT-2 pilot では，proxy error は MAE $=0.0902$，MSE $=0.03227$ まで増加した．
これは contrastive framing が patch-effect target の近似を難しくしている可能性を示す．
ただし，contrastive profile は TF-IDF baseline が依然として完全分類できるため，検出器比較としてはまだ不十分である．

\begin{table}[t]
\centering
\caption{Colab A100 上の GPT-2 residual patching pilot における proxy prediction error．修正前 detector 成績は label leakage のため無効化した．}
\label{tab:gpt2-pilots}
\begin{tabular}{lrrr}
\toprule
Profile & Patched examples & MAE & MSE \\
\midrule
Stress & 24 & 0.0277 & 0.00407 \\
Matched & 24 & 0.0528 & 0.01265 \\
Contrastive & 24 & 0.0902 & 0.03227 \\
\bottomrule
\end{tabular}
\end{table}

この失敗分析を踏まえ，複数の source-boundary frame family を導入した contrastive-frame profile を追加した．
各 row は frame family を表す \texttt{split\_group} を持ち，group split では frame family 全体を test 側に holdout する．
表~\ref{tab:contrastive-frame} に local surrogate の結果を示す．
TF-IDF は F1 $=0.0$，AUROC $=0.599$ まで低下し，ルール特徴および surrogate PatchDistill もほぼ chance となった．
これは，従来の template/order shortcut を抑制した最初の profile であり，修正後の HF detector と patch-distillation 評価を行う次の対象である．

\begin{table}[t]
\centering
\caption{Contrastive-frame profile の group split における local surrogate 結果．}
\label{tab:contrastive-frame}
\begin{tabular}{lrrrr}
\toprule
手法 & F1 & AUROC & FNR & FPR \\
\midrule
TF-IDF + LR & 0.000 & 0.599 & 1.000 & 0.000 \\
Rule features + LR & 0.170 & 0.500 & 0.897 & 0.103 \\
Surrogate PatchDistill & 0.133 & 0.500 & 0.923 & 0.077 \\
\bottomrule
\end{tabular}
\end{table}

修正後 pipeline を用いて，contrastive-frame profile 上で GPT-2 pilot を再実行した．
240 例から one-pass feature を抽出し，32 例に residual stream patching を行った．
Proxy error は MAE $=0.0654$，MSE $=0.01196$ であった．
表~\ref{tab:corrected-gpt2-frame} に detector 結果を示す．
HF features-only detector は AUROC $=0.718$，distilled patch feature を追加した detector は AUROC $=0.755$ となり，ranking metric では改善が見られた．
一方，default threshold $0.5$ では両者とも test example をすべて positive と判定し，FNR $=0.0$ だが FPR $=1.0$ となった．
したがって，現時点で言えることは，distilled patch feature が ranking を改善する可能性を示した一方で，実用的な検出器としては threshold calibration と fixed-FPR 評価が必須である，という点である．

\begin{table}[t]
\centering
\caption{修正後 pipeline による GPT-2 contrastive-frame group split の detector 結果．}
\label{tab:corrected-gpt2-frame}
\begin{tabular}{lrrrr}
\toprule
手法 & F1 & AUROC & FNR & FPR \\
\midrule
HF features only & 0.694 & 0.718 & 0.000 & 1.000 \\
HF features + distilled patch & 0.694 & 0.755 & 0.000 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}

\section{考察}
初期結果から分かることは，MVP，stress，matched，contrastive では表層的手がかりや source-boundary の定型表現が強く，検出器比較としては不十分であるという点である．
また，実装監査により HF detector 評価の label leakage を発見したため，修正前の detector 成績はすべて無効化した．
一方，contrastive-frame group split は TF-IDF とルール特徴を大きく崩しており，修正後 pipeline の評価対象として妥当である．
修正後 GPT-2 pilot では distilled patch feature が AUROC を改善したが，default threshold では FPR が高すぎるため，fixed-FPR recall を用いた再評価が必要である．
...
\begin{enumerate}
  ...
  \item HF モデルで生成した真の patch signature を，より難しい split と held-out framing で再評価する．
  \item hidden-state-only probe，attention-stat-only detector，PatchDistill without proxy，PatchDistill full の ablation を行う．
\end{enumerate}

次の Colab A100 実験では，固定 FPR 指標と prediction CSV を保存する更新済み pipeline で GPT-2 contrastive-frame を再実行する．
その後，Qwen2.5-0.5B-Instruct，Qwen2.5-1.5B，Gemma-2-2B，7B 級 instruction-tuned model へ段階的に拡張する．

\section{限界}
本稿の現時点の結果は，合成データと小規模な GPT-2 residual patching pilot に基づく予備実験である．
...
今後は，より大きな instruction-tuned model と held-out framing split で真の patch signature を追加収集し，proxy prediction error，OOD robustness，benign hard false positive rate，LLM judge call rate を評価する．
